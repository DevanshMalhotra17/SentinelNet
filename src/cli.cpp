...
CLIOptions CLIParser::parse(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--help" || arg == "-h") {
            ...
            options.ports = {21, 22, 23, 25, 80, 135, 139, 443, 445, 3306, 3389, 8080};
        }
...
std::vector<int> CLIParser::parsePortList(const std::string& portStr) {
    ...
    while (std::getline(ss, item, ',')) {
        try {
            ...
            std::cerr << "Warning: Invalid port '" << item << "' ignored." << std::endl;
        }
    }
...
void CLIParser::printHelp() {
    ...
    std::cout << "  -D, --dashboard [port]  Start web dashboard (default: 8080)" << std::endl;
    std::cout << "\nExamples:" << std::endl;
