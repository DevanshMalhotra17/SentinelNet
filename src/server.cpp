...
std::string detectNetworkConfig() {
  ...
  for (const auto &iface : interfaces) {
    int score = 0;
    if (iface.ip.find("127.0.0.1") == 0) continue;
    if (iface.ip.find("169.254.") == 0) continue;
    if (iface.ip.find("192.168.56.") == 0) continue;
    if (iface.ip.find("192.168.57.") == 0) continue;
    if (iface.ip.find("192.168.99.") == 0) continue;
    if (iface.ip.find("10.0.0.") == 0) score = 100;
    else if (iface.ip.find("192.168.1.") == 0) score = 90;
    else if (iface.ip.find("192.168.") == 0) score = 80;
    else if (iface.ip.find("10.") == 0) score = 70;
    if (score > bestScore) { bestScore = score; bestIP = iface.ip; }
  }
...
std::string readFile(const std::string &filepath) {
  std::cout << "  [DEBUG] Attempting to read: " << fs::absolute(filepath) << std::endl;
...
void APIServer::start() {
  ...
  while (isRunning) {
    ...
    if (request.empty()) { closesocket(clientSocket); continue; }
    ...
    if (request.find("GET /hack.js") != std::string::npos) {
      ...
      if (js.empty()) js = readFile("../web/hack.js");
      if (js.empty()) response = "HTTP/1.1 404 Not Found\r\n\r\n404 - hack.js not found";
      ...
      if (html.empty()) html = readFile("../web/hack.html");
      if (html.empty()) response = "HTTP/1.1 404 Not Found\r\n\r\n404 - Hack page not found";
      ...
      if (html.empty()) {
        ...
        // Detect if viewer is remote (PC1) or local (PC2)
        sockaddr_in clientAddr = {};
        int addrLen = sizeof(clientAddr);
        getpeername(clientSocket, (sockaddr*)&clientAddr, &addrLen);
        char clientIP[INET_ADDRSTRLEN];
        inet_ntop(AF_INET, &clientAddr.sin_addr, clientIP, sizeof(clientIP));
        bool isRemote = (std::string(clientIP) != "127.0.0.1");
        std::string injection = "<script>window.IS_REMOTE_VIEWER=" + std::string(isRemote ? "true" : "false") + ";</script>";
      ...
      if (css.empty()) css = readFile("../web/style.css");
      if (css.empty()) css = readFile("sentinelnet/web/style.css");
      ...
      if (js.empty()) js = readFile("../web/script.js");
      if (js.empty()) js = readFile("sentinelnet/web/script.js");
      response = "HTTP/1.1 200 OK\r\nContent-Type: application/javascript\r\n\r\n" + js;

    } else if (request.find("GET /hack1.js") != std::string::npos) {
      std::string js = readFile("hack1.js");
      if (js.empty()) js = readFile("../web/hack1.js");
      ...
      std::string json = "{\"gateway\":\"" + networkConfig.gateway + "\",\"network\":\"" + networkConfig.subnet + "\"}";
      response = "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nAccess-Control-Allow-Origin: *\r\n\r\n" + json;

    } else if (request.find("GET /api/info") != std::string::npos) {
      std::string localIP = getLocalIP();
      std::string json = "{\"host\":\"" + localIP + "\",\"port\":" + std::to_string(serverPort) + "}";
      response = "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nAccess-Control-Allow-Origin: *\r\n\r\n" + json;
      ...
      try {
        NetworkScanner scanner;
        scanner.isHostAlive(target, 1000);
      ...
      if (bodyStart == std::string::npos) {
        ...
        if (portPos != std::string::npos) {
          ...
          if (colonPos != std::string::npos) {
            ...
            std::string portStr = "";
            while (numStart < body.length() && isdigit(body[numStart])) portStr += body[numStart++];
        ...
        if (target.empty() || port == 0) {
          ...
          try {
            ...
            auto escape = [](std::string &s, const std::string &from, const std::string &to) {
              size_t pos = 0;
              while ((pos = s.find(from, pos)) != std::string::npos) { s.replace(pos, from.size(), to); pos += to.size(); }
            };
            escape(escapedBanner, "\"", "\\\"");
            escape(escapedBanner, "\n", "\\n");
            escape(escapedBanner, "\r", "\\r");
            std::string json = "{\"port\":" + std::to_string(result.port) + ",\"service\":\"" + result.service + "\",\"banner\":\"" + escapedBanner + "\"}";
            response = "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nAccess-Control-Allow-Origin: *\r\n\r\n" + json;
          } catch (...) {
      ...
      std::string localIP = getLocalIP();
      std::string json = "{\"version\":\"2.0\",\"host\":\"" + localIP + "\",\"port\":" + std::to_string(serverPort) + ",\"uptime\":" + std::to_string(uptimeSec) + ",\"scans\":" + std::to_string(scanResults.size()) + "}";
      response = "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nAccess-Control-Allow-Origin: *\r\n\r\n" + json;
      ...
      inp[2].type = INPUT_MOUSE; inp[2].mi.dwFlags = MOUSEEVENTF_LEFTUP;
